...
const schema = yup
	...
	.shape({
		...
		email: yup.string().email('Please insert your valid email address').required('Please insert your valid email address'),
		password: yup.string().min(6, 'Password should have atleast 6 characters').required('Please insert your password'),
...
const RegisterForm: React.FC<Props> = ({ children, onSubmit, className, style }) => {
	...
	const [mutation_loginUser, { loading }] = useMutation(
		...
		{
			onCompleted: data => {
				console.log('USER ACCOUNT SUCCESSFULLY CREATED: ', JSON.stringify(data));
				onSubmit();
	...
	return (
		<Box style={style} className={classNames(className ? className : '')}>
			<BaseForm
				...
				/>
				<PJSTextInput name='email' label='Email' error={methods.formState?.errors?.email?.message} placeholder='Email' />
				<PJSTextInput
					name='password'
					label='Password'
